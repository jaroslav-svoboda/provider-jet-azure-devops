...
import (
	...
	"github.com/crossplane-contrib/provider-jet-azuredevops/apis/v1alpha1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		azuredevopsCreds := map[string]string{}
		if err := json.Unmarshal(data, &azuredevopsCreds); err != nil {
		...
		ps.Env = []string{
			fmt.Sprintf(fmtEnvVar, envToken, azuredevopsCreds[keyToken]),
		}
		return ps, nil
	}
}
